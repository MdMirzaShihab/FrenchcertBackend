...
exports.getField = async (req, res) => {
  try {
    ...
    // Manually populate companies since they reference the field
    const companies = await Company.find({ fields: field._id }, "name category");
    
    // Create a new object with the field data and companies
    const responseData = {
      ...field.toObject(),
      companies
    };
...
exports.updateField = async (req, res) => {
  ...
  try {
    ...
    const field = await Field.findByIdAndUpdate(req.params.id, updateData, {
      new: true,
      runValidators: true,
      session,
    });
...
exports.deleteField = async (req, res) => {
  ...
  try {
    ...
    const [certifications, trainings, companies] = await Promise.all([
      ...
      Company.countDocuments({ fields: { $in: [field._id] } }).session(session),
    ]);
